...
| 15  | ❌  | AWS Organization: Monthly cost per department. How?                                                             | [Cost per departments](#615-cost-per-departments)                                                                                   |                     |
...
| 24  | ❌  | Backup & Archiving system for on-premise. Immediately access within minutes in 6 month, available for 10 years. | 1. Use Storage Gateway: Volume Gateway : Stored volume / ~~Cached volume~~; 2. Use life cycle police to archive data after 6 months |                     |
...
## 6.15 Cost per departments

1. Standardize tags attached to AWS resources (using AWS Organization: Policies[^6.15.1] : **Tag Policies**[^6.15.2]);
2. Manage _compliance_ with tag polices (using AWS **Resource Groups**: Tag Editor console[^6.15.3] / Tagging API[^6.15.4]);

   > [!TIP]
   > Tagging your AWS resources[^6.15.5]

3. Use the tag for cost reports (using `Billing & Cost Management` : `Cost Allocation Tags`)
